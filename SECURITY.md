...
Skill/plugin policy must use logical names and reviewed public catalog revisions. Never copy resolved plugin paths, connected-account metadata, or local plugin inventory output. Skill contents belong only in the declared `global/skills/` source trees; keep the catalog manifest limited to metadata. Reconciliation output must remain status-only and sanitized.
