...
For a cross-session handoff, write one short, self-contained, copy-paste-ready message with the objective, essential requirements, verified status, remaining work, and relevant absolute project/worktree/plan paths. Refer to plans as evidence and proposed approaches, not binding authority; direct the receiving agent to verify current state, use independent engineering judgment, and make related improvements for simplicity, consistency, and robustness. State all necessary permissions within the user's authorized scope explicitly, preserve user work and scope limits, and direct autonomous completion without routine confirmation. Unless the user specifies otherwise, implementation handoffs should require an orchestrator using Luna Max (`gpt-5.6-luna`, `max`) or SOL High (`gpt-5.6-sol`, `high`) sub-agents, with integration and final verification owned by the orchestrator. Use `$CODEX_HOME/templates/session-handoff.txt` when its reusable structure helps; keep project-specific implementation detail in the referenced plan.
