...
The reviewed Oracle source is intentionally an explicit user-skill installation rather than an automatic side effect of the core policy installer. Install or update it from `global/skills/oracle-solver` only under explicit user direction, validate it with the system `skill-creator` checks, and then run `./bin/codex-skills-policy verify` to confirm the installed copy exactly matches this snapshot. The invocation policy treats Oracle as a budget-aware planning and problem-solving tool: prefer one early planning call whose guidance can improve a substantial multi-step implementation, use another call when progress is genuinely blocked, and reserve independent review for a specific question with clear value beyond normal evidence and evaluation. It is not a routine post-implementation gate, and one Oracle finding does not automatically require another Oracle call.

Each Oracle run creates a private temporary scratch workspace and launches the pinned worker there with `workspace-write`. Tools and multi-agent workers may freely create, edit, execute, and delete scratch artifacts, while the worker may inspect but not mutate the requested target. The runner alone creates or replaces one marked Markdown response document at its exact target path, emits a concise path-bearing handoff, and then removes the complete scratch workspace. It sets no Oracle timeout; the caller waits with result-check intervals of 1, 2, 4, 8, 16, and then at most 20 minutes. The verdict is an independent judgment for the evidence, constraints, and conditions supplied to that invocation, not a claim of infallibility when evidence or conditions change.
